...
namespace MyControlLibrary
{
    ...
    public partial class OldWScrollBar : UserControl
    {
       
        public OldWScrollBar()
        {
            InitializeComponent();
        ...
        */
        ...
        public delegate void ChatWEvent(int cout, OldWScrollBar sc);
        ...
        public event ChatWEvent MouseorWheelMoveTC;
        ...
        private void Mouse_WhellLoc(object sender,MouseEventArgs e)
        {
            ...
            if (this.SlideBar.Location.X >= 0 && this.SlideBar.Location.X <= Height - SlideBar.Width)
            {
                this.SlideBar.Location = new Point(this.SlideBar.Location.X + xp, this.SlideBar.Location.Y);
                if (this.SlideBar.Location.X < 0)
                {
                    this.SlideBar.Location = new Point(0, this.SlideBar.Location.Y);
                }
                else if (this.SlideBar.Location.X > Height - SlideBar.Width)
                {
        ...
        private void LoSocess()
        {
            ...
            {
                xer += s3.Width;
                if (h.Location.X + h.Width < s3.Location.X + s3.Width)
                    h = s3;
                
            }

            if (h.Location.X + h.Width < Rexc.Width)
            {
                xer = 0;
            }
        ...
        private void SlideBar_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button.ToString().Equals("Left"))
            {
                ...
                if (this.SlideBar.Location.X + e.X - pLeft >= 0 && this.SlideBar.Location.X + e.X - pLeft <= Width - SlideBar.Width)
                {
                    /*
                    int xg = 0;
                    if (this.SlideBar.Location.Y != 0)
                    ...
                    foreach (Control c in h)
                        c.Location = new Point(c.Location.X, c.Location.Y + y);
                    */
                    int df = ((int)od * this.SlideBar.Location.X);
                    Control[] h = sc();
                    ...
                    for (int i = 0; i < co.Length; i++)
                    {
                        int f = co[i];
                        Control j = h[i];
                        j.Location = new Point(f - df, j.Location.Y);
                    }

                    this.SlideBar.Location = new Point(this.SlideBar.Location.X + e.X - pLeft, SlideBar.Location.Y);
                }
            }
            if (MouseorWheelMoveTC != null)
                MouseorWheelMoveTC(this.SlideBar.Location.X, this);
